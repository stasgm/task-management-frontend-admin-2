...
import { discussionGenerator, taskGenerator, userGenerator } from './data-generators';
...
export const createTask = async (discussionProperties?: any) => {
  const discussion = taskGenerator(discussionProperties);
  const res = await db.task.create(discussion);
  return res;
};
